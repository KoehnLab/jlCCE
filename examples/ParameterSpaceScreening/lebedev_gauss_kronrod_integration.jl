...
function get_mod_depth(alpha,R,R12,B0)
    ...
    return isnan(lambda) || isinf(lambda) ? 0.0 : lambda
end
...
function get_nzq_frequency(alpha,R,R12,B0)
    ...
    return isnan(w12) || isinf(w12) ? 0.0 : w12 
end
...
# run Gauss-Kronrod integration and Lebedev integration depending on r,r12 - averaging of alpha and the magnetic field direction
df = DataFrame(r=Float64[],r12=Float64[], averaged_modulation_depth=Float64[], averaged_nzq_frequency=Float64[], time_hahn_echo=String[])
x,y,z,w = lebedev_by_order(5)
n_points = size(x)
...
for k = 1:length(r)[1]
    for l = 1:length(r12)[1]
        ...
        for i in eachindex(x)
            ...
            # if any(isnan.(B0)) || any(isinf.(B0)) || norm(B0) ≈ 0
                # @warn "Ungültiger B0-Vektor" B0 x[i] y[i] z[i]
                # continue
            # end

            int_moddepth, err_mod = quadgk(alpha -> get_mod_depth(alpha, R, R12, B0), 0, 2π, rtol=1e-8)
            modulation_depth = int_moddepth/(2*pi)
            averaged_modulation_depth += w[i] * modulation_depth
            #println("averaged mod. depth: ",averaged_modulation_depth)

            int_freq, err_freq = quadgk(alpha -> get_nzq_frequency(alpha,R,R12,B0), 0, 2*pi, rtol=1e-8)
            nzq_frequency = int_freq/(2*pi)
        ...
        push!(df, (R, R12, averaged_modulation_depth, averaged_nzq_frequency, join(time_hahn_echo,",")))

    end
end

CSV.write("results_gauss_kronrod_lebedev_integration_alphaandB0_order_5.csv", df)
